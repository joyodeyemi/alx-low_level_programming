...
In this project, I learned the three standard file descriptors and their
`POSIX` names, the difference between function and system calls.
## Header File :file_folder:

* [main.h](./main.h): The Header file contains prototypes for all functions
...
* **0. Tread lightly, she is near**
  ...
  * If the file is `NULL` or cannot be opened or read it will return `0`.
  * If the `write` call fails or does not write the expected number of bytes it will return `0`.
  * Otherwise, it will return the actual number of bytes the function can read and print.
...
* **1. Under the snow**
  ...
  * If `filename` is `NULL` or the funciton fails it will return `-1`.
...
* **2. Speak gently, she can hear**
  ...
  * If the function fails or `filename` is `NULL` it will return `-1`.
  * If the file does not exist or the user lacks write permissions on the file it will return `-1`.
